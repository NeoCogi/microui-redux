...
use common::{
    ...
    polymesh::PolyMesh,
    view3d::View3D,
    vulkan_renderer::{CustomRenderArea, MeshBuffers, MeshSubmission, MeshVertex, VulkanRenderer},
};
#[cfg(feature = "builder")]
...
struct State<'a> {
    renderer: RendererHandle<VulkanRenderer>,
...
impl<'a> State<'a> {
    pub fn new(backend: BackendInitContext, slots: Vec<SlotId>, ctx: &mut Context<VulkanRenderer>) -> Self {
        ...
        let white_uv = renderer.scope(|vk| {
            let atlas = vk.get_atlas();
    ...
    fn style_window(&mut self, ctx: &mut Context<VulkanRenderer>) {
    ...
    fn log_window(&mut self, ctx: &mut Context<VulkanRenderer>) {
    ...
    fn triangle_window(&mut self, ctx: &mut Context<VulkanRenderer>) {
    ...
    fn suzane_window(&mut self, ctx: &mut Context<VulkanRenderer>) {
            ...
            container.with_row(&[SizePolicy::Remainder(0)], SizePolicy::Remainder(0), |container| {
                container.custom_render_widget("Suzane", WidgetOption::HOLD_FOCUS, move |_dim, cra| {
                    ...
                    if let Ok(mut suzane) = suzane_state.write() {
                        ...
                        renderer.scope_mut(|vk| {
                            vk.enqueue_mesh_draw(area, submission.clone());
                        });
                    }
                });
            });
    ...
    fn dialog(&mut self, ctx: &mut Context<VulkanRenderer>) {
        if self.open_dialog {
    ...
    fn process_frame(&mut self, ctx: &mut Context<VulkanRenderer>) {
        let now = Instant::now();
...
fn main() {
    ...
    let mut app = Application::new(atlas.clone(), move |backend: BackendInitContext, ctx| {
        let slots = atlas.clone_slot_table();
        let mut state = State::new(backend, slots, ctx);
